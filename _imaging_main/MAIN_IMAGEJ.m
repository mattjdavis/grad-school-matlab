...
[total_frames, num_rois] =size(Read);
num_rois=num_rois/4;
num_trials=total_frames/num_frames;
...
for k = 1:num_days
    ...
    % df/f
    raw_fluor= reshape(read_data,num_frames,num_trials,num_rois);
    bl_vec=raw_fluor(baseline_frames,:,:);
    mean_baseline=mean(bl_vec);
    bl=repmat(mean_baseline,num_frames,1);
    ...
    % roi_tags, roi_labels
    % need to fill to be consistent with sima imports
    roi_tags={};
    roi_labels=1:num_rois;
